...
def resnet50(pretrained=False, num_classes=3,**kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=3,**kwargs)

    if pretrained:
        model_dict = model.state_dict()
        pretrained_model = model_zoo.load_url(model_urls['resnet50'])

        pretrained_model = {k: v for k, v in pretrained_model.items() if (k in model_dict and 'fc' not in k)}

        model_dict.update(pretrained_model)
        model.load_state_dict(model_dict)
    model.fc = nn.Linear(150528,num_classes)
    return model
...
if __name__ == '__main__':
    ...
    print(model)
    summary(model, (3, 224, 224))  #以类似表格方式打印模型结构以及每层大小