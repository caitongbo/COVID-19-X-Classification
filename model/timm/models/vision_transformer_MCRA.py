...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.fpn4 = nn.MaxPool2d(kernel_size=2, stride=2)
         ...
         # Top layer
        self.toplayer = nn.Conv2d(embed_dim, 256, kernel_size=1, stride=1, padding=0)  # Reduce channels
        ...
        # Lateral layers
        self.latlayer1 = nn.Conv2d(embed_dim, 256, kernel_size=1, stride=1, padding=0)
        self.latlayer1_bn = nn.BatchNorm2d(256)
        self.latlayer1_relu = nn.ReLU(inplace=True)

        self.latlayer2 = nn.Conv2d(embed_dim, 256, kernel_size=1, stride=1, padding=0)
        self.latlayer2_bn = nn.BatchNorm2d(256)
        self.latlayer2_relu = nn.ReLU(inplace=True)

        self.latlayer3 = nn.Conv2d(embed_dim, 256, kernel_size=1, stride=1, padding=0)
        ...
        self.classifier = MHA(input_dim=embed_dim, num_heads=1, num_classes=3, lam=0.1)

        self.loss_func = F.binary_cross_entropy_with_logits
        self.head_dist = None
        if distilled:
            self.head_dist = nn.Linear(3072, self.num_classes) if num_classes > 0 else nn.Identity()
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        self.num_classes = num_classes
	        # We add our MHA (CSRA) beside the orginal VIT structure below
        self.head = nn.Sequential() # delete original classifier
        self.classifier = MHA(input_dim=3072, num_heads=1, num_classes=3, lam=0.1)
    ...
    def backbone(self, x):
        ...
        # p2 = self.fpn1(x) # 32 758 56x56
        # p3 = self.fpn2(x) # 28x28
        # p4 = self.fpn3(x) # 14x14
        # p5 = self.fpn4(x) # 7x7
        ...
        c2 = self.fpn1(x) # 32 758 56x56
        c3 = self.fpn2(x) # 28x28
        c4 = self.fpn3(x) # 14x14
        c5 = self.fpn4(x) # 7x7
